...
export class CollectorShardAssignmentService {
  ...
  async assign(symbols: readonly string[]): Promise<CollectorShardAssignment> {
    ...
    const activeCollectors = await this.activeCollectorIds();
  ...
  private async activeCollectorIds(): Promise<string[]> {
    const fallback = [this.runtime.workerInstanceId];
    ...
    try {
      ...
      for (let i = 0; i < keys.length; i += 1) {
        ...
        if (!payload || !Array.isArray(payload.roles) || !payload.roles.includes('collector')) {
          continue;
        }
