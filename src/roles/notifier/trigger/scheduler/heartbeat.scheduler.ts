...
import { HeartbeatUsecase } from '@roles/notifier/usecase/heartbeat.usecase';
...
export class HeartbeatScheduler implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    ...
    this.timer.unref();
    this.logger.log(`scheduler notifier.heartbeat every ${this.redis.heartbeatIntervalSec}s`);
  }
  ...
  async tick(): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(`notifier heartbeat failed: ${err instanceof Error ? err.message : err}`);
    } finally {
      this.running = false;
    }
  }
