...
@Injectable()
export class IngestDecisionMadeUsecase {
  private readonly logger = new Logger(IngestDecisionMadeUsecase.name);
  ...
  async execute(input: {
    ...
    const occurredAt = new Date(input.payload.decidedAt);
    const resolved = resolveEventType(DECISION_MADE_STREAM, input.payload);
    ...
    if (!isNew) {
      this.logger.debug(
        `decision.made duplicate suppressed (entry=${input.streamEntryId} account=${input.payload.accountExternalId})`,
      );
