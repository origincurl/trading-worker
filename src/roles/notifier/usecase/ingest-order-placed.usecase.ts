...
@Injectable()
export class IngestOrderPlacedUsecase {
  private readonly logger = new Logger(IngestOrderPlacedUsecase.name);
  ...
  async execute(input: {
    ...
    const occurredAt = new Date(input.payload.placedAt);
    const resolved = resolveEventType(ORDER_PLACED_STREAM, input.payload);
    ...
    if (!isNew) {
      this.logger.debug(
        `order.placed duplicate suppressed (entry=${input.streamEntryId} externalOrderId=${input.payload.externalOrderId})`,
      );
